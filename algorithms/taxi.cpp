...
int main() {
    ...
    for(size_t i = 0; i < m; ++i) {
        std::cin >> hour >> placeholder >> minute >> a >> b >> c >> d;
        time = hour * 60 + minute;
        orders[i] = {time, {a, b}, {c, d}};
    }
    ...
    for (size_t i = 0; i < orders.size() - 1; ++i) {
        ...
        for(size_t j = i + 1; j < orders.size(); ++j) {
            auto to = orders[j];
            if (from.time + from.duration() + get_duration(from.arrival, to.departure) < to.time) {
                g.add_edge(i, m + j);
            }
        }
    }
